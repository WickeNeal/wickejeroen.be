...
<template>
    <div ref="ribbonsContainer" class="relative h-full w-full overflow-hidden" />
</template>
...
const createLines = () => {
    ...
    props.colors.forEach((color, index) => {
        ...
        const mouseOffset = new Vec3((index - center) * props.offsetFactor + (Math.random() - 0.5) * 0.01, (Math.random() - 0.5) * 0.1, 0);
...
const initRibbons = () => {
    ...
    if (Array.isArray(props.backgroundColor) && props.backgroundColor.length === 4) {
        gl.clearColor(props.backgroundColor[0], props.backgroundColor[1], props.backgroundColor[2], props.backgroundColor[3]);
    } else {
