...
<template>
    <div class="relative aspect-square w-1/2 max-md:w-11/12" :style="wrapperStyle">
        <div ref="sceneRef" class="grid h-full w-full" :style="sceneStyle">
            <template v-for="(_, r) in cells" :key="`row-${r}`">
                <div
                    v-for="(__, c) in cells"
                    :key="`${r}-${c}`"
                    class="cube relative aspect-square h-full w-full [transform-style:preserve-3d]"
                    :data-row="r"
                    :data-col="c"
                >
                    <span class="pointer-events-none absolute -inset-9" />
...
const sceneStyle = computed(() => ({
    gridTemplateColumns: props.cubeSize ? `repeat(${props.gridSize}, ${props.cubeSize}px)` : `repeat(${props.gridSize}, 1fr)`,
    gridTemplateRows: props.cubeSize ? `repeat(${props.gridSize}, ${props.cubeSize}px)` : `repeat(${props.gridSize}, 1fr)`,
