...
const setup = () => {
    ...
    const bloomPass = markRaw(new UnrealBloomPass(new THREE.Vector2(1, 1), props.bloomStrength, props.bloomRadius, props.bloomThreshold));
    ...
    const resize = () => {
        ...
        const currentDPR = Math.min(typeof window !== 'undefined' ? window.devicePixelRatio || 1 : 1, props.maxDevicePixelRatio);
        const need = cssW * cssH * currentDPR * currentDPR;
        const scale = need <= pixelBudget.value ? 1 : Math.max(0.5, Math.min(1, Math.sqrt(pixelBudget.value / Math.max(1, need))));
    ...
    const animate = () => {
        ...
        if (pointerActiveRef.value) {
            velocityRef.value.set(currentMouseRef.value.x - mat.uniforms.iMouse.value.x, currentMouseRef.value.y - mat.uniforms.iMouse.value.y);
