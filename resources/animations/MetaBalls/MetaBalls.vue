...
const setupAnimation = () => {
    ...
    function update(time: number) {
        ...
        const targetX = pointerInside ? pointerX : gl.canvas.width * 0.5 + Math.cos(elapsed * props.speed) * gl.canvas.width * 0.15;
        const targetY = pointerInside ? pointerY : gl.canvas.height * 0.5 + Math.sin(elapsed * props.speed) * gl.canvas.height * 0.15;
...
<template>
    <div ref="containerRef" class="relative h-full w-full" :style="`mix-blend-mode: ${props.mixBlendMode}`" />
</template>
