...
const blurDivs = computed(() => {
    ...
    const currentStrength =
        isHovered.value && config.value.hoverIntensity ? config.value.strength * config.value.hoverIntensity : config.value.strength;
...
<template>
    <div
        ref="containerRef"
        :class="['gradual-blur relative isolate', config.target === 'page' ? 'gradual-blur-page' : 'gradual-blur-parent', config.className]"
