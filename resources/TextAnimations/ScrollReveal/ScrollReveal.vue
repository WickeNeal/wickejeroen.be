...
<template>
    <h2 ref="containerRef" :class="`my-5 ${containerClassName}`">
        <p :class="`leading-relaxed font-semibold ${textClassName}`" style="font-size: clamp(1.6rem, 4vw, 3rem)">
            <span v-for="(word, index) in splitText" :key="index" :class="word.isWhitespace ? '' : 'word inline-block'">
                {{ word.text }}
            </span>
        </p>
    </h2>
...
const initializeAnimation = () => {
    ...
    const scroller = props.scrollContainerRef && props.scrollContainerRef.current ? props.scrollContainerRef.current : window;
