...
<template>
    <div ref="containerRef" class="relative h-full w-full overflow-hidden bg-transparent">
        <h1
            ref="titleRef"
            :class="`text-pressure-title ${className} ${flex ? 'flex justify-between' : ''} ${stroke ? 'stroke' : ''} text-center uppercase`"
            :style="titleStyle"
        >
            <span v-for="(char, i) in chars" :key="i" :ref="(el) => (spansRef[i] = el as HTMLSpanElement)" :data-char="char" class="inline-block">
                {{ char }}
            </span>
        </h1>
    </div>
