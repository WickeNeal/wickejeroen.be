...
<template>
    <div class="sm-scope h-full w-full">
        <div
            :class="(className ? className + ' ' : '') + 'staggered-menu-wrapper relative z-40 h-full w-full'"
            :style="accentColor ? { '--sm-accent': accentColor } : undefined"
            :data-position="position"
            :data-open="open || undefined"
        >
            <div ref="preLayersRef" class="sm-prelayers pointer-events-none absolute top-0 right-0 bottom-0 z-[5]" aria-hidden="true">
                <div
            ...
            <header
                class="staggered-menu-header pointer-events-none absolute top-0 left-0 z-20 flex w-full items-center justify-between bg-transparent p-[2em]"
                aria-label="Main navigation header"
            >
                <div class="sm-logo pointer-events-auto flex items-center select-none" aria-label="Logo">
                    <img
                        :src="logoUrl || '/src/assets/logos/reactbits-gh-white.svg'"
                        alt="Logo"
                        class="sm-logo-img block h-8 w-auto object-contain"
                        :draggable="false"
                        width="110"
                        height="24"
                    />
                ...
                <button
                    ref="toggleBtnRef"
                    class="sm-toggle pointer-events-auto relative inline-flex cursor-pointer items-center gap-[0.3rem] overflow-visible border-0 bg-transparent leading-none font-medium text-[#e9e9ef]"
                ...
                >
                    <span
                        ref="textWrapRef"
                        class="sm-toggle-textWrap relative inline-block h-[1em] w-[var(--sm-toggle-width,auto)] min-w-[var(--sm-toggle-width,auto)] overflow-hidden whitespace-nowrap"
                        aria-hidden="true"
                    >
                        <span ref="textInnerRef" class="sm-toggle-textInner flex flex-col leading-none">
                            <span v-for="(line, index) in textLines" :key="index" class="sm-toggle-line block h-[1em] leading-none">
                                {{ line }}
                            </span>
                        </span>
                    </span>
                    ...
                    >
                        <span
                            ref="plusHRef"
                            class="sm-icon-line absolute top-1/2 left-1/2 h-[2px] w-full -translate-x-1/2 -translate-y-1/2 rounded-[2px] bg-current [will-change:transform]"
                        />
                        <span
                            ref="plusVRef"
                            class="sm-icon-line sm-icon-line-v absolute top-1/2 left-1/2 h-[2px] w-full -translate-x-1/2 -translate-y-1/2 rounded-[2px] bg-current [will-change:transform]"
                        />
                    </span>
                </button>
            </header>
            ...
            <aside
                ...
                ref="panelRef"
                class="staggered-menu-panel absolute top-0 right-0 z-10 flex h-full flex-col overflow-y-auto bg-white p-[6em_2em_2em_2em] backdrop-blur-[12px]"
            ...
            >
                <div class="sm-panel-inner flex flex-1 flex-col gap-5">
                    <ul class="sm-panel-list m-0 flex list-none flex-col gap-2 p-0" role="list" :data-numbering="displayItemNumbering || undefined">
                        ...
                        <li v-else class="sm-panel-itemWrap relative overflow-hidden leading-none" aria-hidden="true">
                            <span
                                class="sm-panel-item relative inline-block cursor-pointer pr-[1.4em] text-[4rem] leading-none font-semibold tracking-[-2px] text-black uppercase no-underline transition-[background,color] duration-150 ease-linear"
                            >
                                <span class="sm-panel-itemLabel inline-block [transform-origin:50%_100%] will-change-transform"> No items </span>
                            </span>
                        </li>
                    </ul>
                    ...
                    >
                        ...
                        <ul class="sm-socials-list m-0 flex list-none flex-row flex-wrap items-center gap-4 p-0" role="list">
