...
<template>
    <div :style="{ transform: `scale(${props.size})` }" :class="props.class">
        <div :class="folderClass" :style="folderStyle" @click="handleClick">
            <div
                class="rounded-tl-0 relative h-[80px] w-[100px] rounded-tr-[10px] rounded-br-[10px] rounded-bl-[10px]"
                :style="{ backgroundColor: folderBackColor }"
            >
                <span
                    class="rounded-bl-0 rounded-br-0 absolute bottom-[98%] left-0 z-0 h-[10px] w-[30px] rounded-tl-[5px] rounded-tr-[5px]"
                    :style="{ backgroundColor: folderBackColor }"
                ></span>
...
const folderClass = computed(() =>
    `group relative transition-all duration-200 ease-in cursor-pointer ${!open.value ? 'hover:-translate-y-2' : ''}`.trim(),
);
