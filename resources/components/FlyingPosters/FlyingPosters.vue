...
<template>
    <div ref="containerRef" :class="['relative z-2 h-full w-full overflow-hidden', className]" v-bind="$attrs">
...
function AutoBind<T extends object>(self: T, { include, exclude }: AutoBindOptions = {}) {
    ...
    const filter = (key: string | symbol) => {
        const match = (pattern: string | RegExp) => (typeof pattern === 'string' ? key === pattern : (pattern as RegExp).test(key.toString()));
    ...
    for (const [object, key] of getAllProperties(proto)) {
        ...
        if (descriptor && typeof descriptor.value === 'function' && typeof key === 'string') {
            ...
            if (typeof current === 'function') {
                (self as Record<MethodNames<T>, unknown>)[key as MethodNames<T>] = (current as (...a: unknown[]) => unknown).bind(self);
            }
        }
    }
    return self;
...
watch(
    () => [props.items, props.planeWidth, props.planeHeight, props.distortion, props.scrollEase, props.cameraFov, props.cameraZ],
    () => {
        destroyCanvas();
        initCanvas();
    },
