    ...
    <div ref="containerRef" :class="`relative w-[500px] ${className}`.trim()">
        <div
            ...
            :class="`max-h-[400px] overflow-y-auto p-4 ${
                displayScrollbar
                    ? '[&::-webkit-scrollbar]:w-[8px] [&::-webkit-scrollbar-thumb]:rounded-[4px] [&::-webkit-scrollbar-thumb]:bg-[#222] [&::-webkit-scrollbar-track]:bg-[#0b0b0b]'
                    : 'scrollbar-hide'
            }`"
            :style="{
        ...
        >
            <Motion
                ...
                "
            >
                <div :class="`rounded-lg bg-[#111] p-4 ${selectedIndex === index ? 'bg-[#222]' : ''} ${itemClassName}`">
        ...
        <div
            v-if="showGradients"
            class="ease pointer-events-none absolute top-0 right-0 left-0 h-[50px] bg-gradient-to-b from-[#0b0b0b] to-transparent transition-opacity duration-300"
            :style="{ opacity: topGradientOpacity }"
        />
        <div
            v-if="showGradients"
            class="ease pointer-events-none absolute right-0 bottom-0 left-0 h-[100px] bg-gradient-to-t from-[#0b0b0b] to-transparent transition-opacity duration-300"
