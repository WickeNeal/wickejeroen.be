...
<template>
    <div
        ref="rootRef"
        class="relative flex h-full w-full flex-wrap items-start justify-center gap-3"
        :style="{
            '--r': `${props.radius}px`,
            '--x': '50%',
            '--y': '50%',
    ...
    >
        <article
            v-for="(c, i) in data"
            :key="i"
            class="group relative flex w-[300px] flex-col overflow-hidden rounded-[20px] border border-[#333] transition-all duration-300 hover:border-[var(--card-border)]"
            :style="{
                '--mouse-x': '50%',
                '--mouse-y': '50%',
        ...
        >
            ...
            <footer class="relative z-10 grid grid-cols-[1fr_auto] gap-x-3 gap-y-1 p-3 font-sans text-white">
        ...
        <div ref="fadeRef" class="pointer-events-none absolute inset-0 z-40 transition-opacity duration-[250ms]" :style="fadeStyle" />
    </div>
</template>
