...
<template>
    ...
    >
        <div
            ...
            :key="idx"
            ref="cardRefs"
            class="absolute aspect-square w-[200px] overflow-hidden rounded-[25px] border-[5px] border-white bg-[#0b0b0b] opacity-0 shadow-[0_4px_10px_rgba(0,0,0,0.2)]"
            :style="{ transform: transformStyles[idx] ?? 'none' }"
            @mouseenter="() => pushSiblings(idx)"
            @mouseleave="resetSiblings"
        >
            <div v-if="!imageLoaded[idx]" class="shimmer-container absolute inset-0 z-[1] overflow-hidden bg-[#0b0b0b]"></div>

            <img
                class="absolute inset-0 z-[2] h-full w-full object-cover transition-opacity duration-700 ease-out"
                :src="src"
                :alt="`card-${idx}`"
