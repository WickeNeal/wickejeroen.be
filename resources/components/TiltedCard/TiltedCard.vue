...
<template>
    ...
    >
        <div v-if="showMobileWarning" class="absolute top-4 block text-center text-sm sm:hidden">
        ...
        >
            <img
                :src="imageSrc"
                :alt="altText"
                class="absolute top-0 left-0 [transform:translateZ(0)] rounded-[15px] object-cover will-change-transform"
                :style="{
                    width: imageWidth,
                    height: imageHeight,
                }"
            ...
            <Motion
                ...
                tag="div"
                class="absolute top-0 left-0 z-[2] [transform:translateZ(30px)] will-change-transform"
            >
                <slot name="overlay" />
            </Motion>
        </Motion>
        ...
        <Motion
            ...
            tag="figcaption"
            class="pointer-events-none absolute top-0 left-0 z-[3] hidden rounded-[4px] bg-white px-[10px] py-[4px] text-[10px] text-[#2d2d2d] opacity-0 sm:block"
            :animate="{
                x: xValue,
                y: yValue,
