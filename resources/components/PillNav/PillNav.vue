...
<template>
    <div class="absolute top-[1em] left-0 z-[1000] w-full md:left-auto md:w-auto">
        <nav
            :class="['box-border flex w-full items-center justify-between px-4 md:w-max md:justify-start md:px-0', className]"
            aria-label="Primary"
            :style="cssVars"
        >
            <component
                ...
                role="menuitem"
                ref="logoRef"
                class="inline-flex items-center justify-center overflow-hidden rounded-full p-2"
                :style="{
                    width: 'var(--nav-h)',
                    height: 'var(--nav-h)',
            ...
            >
                <img :src="logo" :alt="logoAlt" ref="logoImgRef" class="block h-full w-full object-cover" />
            </component>

            <div
                ref="navItemsRef"
                class="relative ml-2 hidden items-center rounded-full md:flex"
                :style="{
                    height: 'var(--nav-h)',
                    background: 'var(--base, #000)',
                }"
            >
                <ul role="menubar" class="m-0 flex h-full list-none items-stretch p-[3px]" :style="{ gap: 'var(--pill-gap)' }">
                    <li v-for="(item, i) in items" :key="item.href || `item-${i}`" class="flex h-full" role="none">
                        <component
                            ...
                            class="relative box-border inline-flex h-full cursor-pointer items-center justify-center overflow-hidden rounded-full px-0 text-[16px] leading-[0] font-semibold tracking-[0.2px] whitespace-nowrap uppercase no-underline"
                            :style="{
                        ...
                        >
                            <span
                                class="hover-circle pointer-events-none absolute bottom-0 left-1/2 z-[1] block rounded-full"
                                :style="{
                                    background: 'var(--base, #000)',
                            ...
                            />
                            <span class="label-stack relative z-[2] inline-block leading-[1]">
                                <span class="pill-label relative z-[2] inline-block leading-[1]" :style="{ willChange: 'transform' }">
                                    {{ item.label }}
                                </span>
                                <span
                                    class="pill-label-hover absolute top-0 left-0 z-[3] inline-block"
                                    :style="{
                            ...
                            <span
                                ...
                                class="absolute -bottom-[6px] left-1/2 z-[4] h-3 w-3 -translate-x-1/2 rounded-full"
            ...
            <button
                ...
                class="relative flex cursor-pointer flex-col items-center justify-center gap-1 rounded-full border-0 p-0 md:hidden"
                :style="{
                    width: 'var(--nav-h)',
                    height: 'var(--nav-h)',
                    background: 'var(--base, #000)',
            ...
            >
                <span
                    class="hamburger-line h-0.5 w-4 origin-center rounded transition-all duration-[10ms] ease-[cubic-bezier(0.25,0.1,0.25,1)]"
        ...
        <div
            ref="mobileMenuRef"
            class="absolute top-[3em] right-4 left-4 z-[998] origin-top rounded-[27px] shadow-[0_8px_32px_rgba(0,0,0,0.12)] md:hidden"
            :style="{
                ...cssVars,
                background: 'var(--base, #f0f0f0)',
            }"
        >
            <ul class="m-0 flex list-none flex-col gap-[3px] p-[3px]">
                <li v-for="item in items" :key="item.href || `mobile-${item.label}`">
                    <component
                        :is="isRouterLink(item.href) ? 'RouterLink' : 'a'"
                        :to="isRouterLink(item.href) ? item.href : undefined"
                        :href="!isRouterLink(item.href) ? item.href : undefined"
                        class="block rounded-[50px] px-4 py-3 text-[16px] font-medium transition-all duration-200 ease-[cubic-bezier(0.25,0.1,0.25,1)]"
