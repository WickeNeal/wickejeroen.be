...
<template>
    <div
        ref="containerRef"
        :class="['relative overflow-hidden p-4', round ? 'rounded-full border border-[#333]' : 'rounded-[24px] border border-[#333]']"
        :style="{
            width: `${baseWidth}px`,
    ...
    >
        ...
        >
            <Motion
                ...
                :class="[
                    ...
                    round
                        ? 'items-center justify-center rounded-full border border-[#333] bg-[#111] text-center'
                        : 'items-start justify-between rounded-[12px] border border-[#333] bg-[#111]',
                ]"
                :style="{
                    width: itemWidth + 'px',
        ...
        <div :class="['flex w-full justify-center', round ? 'absolute bottom-12 left-1/2 z-20 -translate-x-1/2' : '']">
...
const rotateYTransforms = Array.from({ length: maxItems }, (_, index) => {
    const range = computed(() => [-(index + 1) * trackItemOffset.value, -index * trackItemOffset.value, -(index - 1) * trackItemOffset.value]);
