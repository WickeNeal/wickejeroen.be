...
<template>
    <div :style="{ height: currentHeight + 'px', scrollbarWidth: 'none' }" class="mx-2 flex max-w-full items-center">
        <div
            ...
            :class="`${props.className} absolute bottom-2 left-1/2 flex w-fit -translate-x-1/2 transform items-end gap-4 rounded-2xl border-2 border-neutral-700 px-4 pb-2`"
...
const DockItem = defineComponent({
    ...
    render() {
        ...
        return h(
            ...
            [h(DockIcon, {}, () => [icon]), h(DockLabel, { isHovered: this.isHovered }, () => [typeof label === 'string' ? label : label])],
        );
    },
});
