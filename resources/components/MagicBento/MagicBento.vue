...
<template>
    ...
    <BentoCardGrid
        ...
        "
    >
        <div class="card-responsive grid gap-2">
            <template v-for="(card, index) in cardData" :key="index">
                ...
                >
                    <div class="card__header relative flex justify-between gap-3 text-white">
                        <span class="card__label text-base">{{ card.label }}</span>
                    </div>
                    <div class="card__content relative flex flex-col text-white">
                        <h3 :class="`card__title m-0 mb-1 text-base font-normal ${textAutoHide ? 'text-clamp-1' : ''}`">
                            {{ card.title }}
                        </h3>
                ...
                <div v-else :class="baseClassName" :style="getCardStyle(card)" :ref="(el) => setupCardRef(el as HTMLDivElement, index)">
                    <div class="card__header relative flex justify-between gap-3 text-white">
                        <span class="card__label text-base">{{ card.label }}</span>
                    </div>
                    <div class="card__content relative flex flex-col text-white">
