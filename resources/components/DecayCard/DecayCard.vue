...
<template>
    <div ref="svgRef" class="relative" :style="{ width: `${width}px`, height: `${height}px` }">
        <svg viewBox="-60 -75 720 900" preserveAspectRatio="xMidYMid slice" class="relative block h-full w-full [will-change:transform]">
            <filter id="imgFilter">
                <feTurbulence
                    type="turbulence"
                    baseFrequency="0.015"
        ...
        <div class="absolute bottom-[1.2em] left-[1em] text-[2rem] leading-[1.5em] font-black tracking-[-0.5px] first-line:text-[4rem]">
            <slot />
        </div>
    </div>
</template>
